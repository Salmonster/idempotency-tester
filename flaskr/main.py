...
def submit():
    ...
    # Here we make sure we don't update the account balance if the idempotency
    # feature is set and the key has been used already
    if key != '':
    ...
    # No idempotency key set, so update the balance no matter what
    else:
        cur.execute(f'UPDATE balance SET universal_balance = {new_balance};')
    conn.commit()
    cur.close()
